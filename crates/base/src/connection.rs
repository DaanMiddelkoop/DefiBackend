...
impl Connection {
    ...
    pub fn send_now(&mut self, cx: &mut Context<'_>, message: Message) -> Result<()> {
    ...
    pub fn poll(&mut self, cx: &mut Context<'_>) -> Poll<Result<Utf8Bytes>> {
        ...
        while let Poll::Ready(x) = self.client.poll_next_unpin(cx).map_err(|x| format!("While receiving: {x}"))? {
            match x {
                Some(message) => {
                    ...
                    self.reset_timeout();
                    match message {
                        Message::Text(bytes) => return Poll::Ready(Ok(bytes)),
                        Message::Binary(_) => todo!(),
                        Message::Ping(items) => self.send_now(cx, Message::Pong(items))?,
                        Message::Pong(_) => (),
                        Message::Close(_) => return Poll::Ready(Err("Connection closed".to_string())),
        ...
        while self.ping.poll_tick(cx).is_ready() {
            self.send_now(cx, Message::Ping(Bytes::from_static("123".as_bytes())))?;
        }
